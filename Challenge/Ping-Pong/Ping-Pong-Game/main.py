...
bar = {
    "w": WINDOW_SIZE[0] * (0.09),
    "h": 100000000,
    "wallSp": 5,
    "speed": 720 / FPS,
    "color": (210, 210, 210),
    "multiAng": WINDOW_SIZE[0] / 100,
...
ball_feat = {
    ...
    "h": 8,
    "defaultSpeed": 3.5,
    "defaultAng": ((WINDOW_SIZE[1] / FPS) / 4),
    "addAng": 3 / FPS,
    "addSpeed": (FPS + 10) / FPS,
    "move": False,
    "dir_s": 0,
...
bar_1 = pygame.Rect(
    main_SIZE[0] // 2 - bar["w"] // 2, bar["wallSp"] - bar["h"], bar["w"], bar["h"]
)
bar_2 = pygame.Rect(
    main_SIZE[0] // 2 - bar["w"] // 2, main_SIZE[1] - bar["wallSp"], bar["w"], bar["h"]
)
...
def random_color() -> tuple:
    return (
        random.randint(DEGRE_COLOR[0], DEGRE_COLOR[1]),
        random.randint(DEGRE_COLOR[0], DEGRE_COLOR[1]),
        random.randint(DEGRE_COLOR[0], DEGRE_COLOR[1]),
    )
...
while run:
    ...
    if not ball_feat["move"]:
        if keys[pygame.K_SPACE]:
            ball_feat["dir_s"] = random.choice(
                [ball_feat["defaultSpeed"], -ball_feat["defaultSpeed"]]
            )
            ball_feat["ang"] = random.choice(
                [ball_feat["defaultAng"], -ball_feat["defaultAng"]]
            )
            ball_feat["move"] = True
    # for bars
